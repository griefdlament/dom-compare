(function(){
   ...
   var fs = require('fs');
   var attributeExclude = [];


   function Comparator(options, collector, attribute) {
      this._options = options || {};
      if(!collector)
         throw new Error("Collector instance must be specified");
      this._collector = collector;

      this._loadAttributeExcludeFile(attribute);
   }

   // mock function for load config file
   Comparator.prototype._loadAttributeExcludeFile = function(path) {
     fs.readFileSync(path).toString().split("\n").forEach(function(line, index, arr) {
       if(line != "")
         attributeExclude.push(line.trim())
     })
   };
   ...
   module.exports = function(a, b, options, attribute) {


      var collector = new Collector(options);
      var comparator = new Comparator(options, collector, attribute);
